...
def get_tasks(user: user_schema.User, is_done: bool = None):
    if(is_done is None):
        tasks_by_user = TodoModel.filter(TodoModel.user_id == user.id).order_by(TodoModel.create_at.desc())
    else:
        tasks_by_user = TodoModel.filter((TodoModel.user_id == user.id) & (TodoModel.is_done == is_done)).order_by(TodoModel.create_at.desc())
    list_tasks = []
